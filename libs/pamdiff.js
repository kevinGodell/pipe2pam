...
PamDiff.prototype._transform = function (chunk, encoding, callback) {
    ...
    if (this._oldPix !== null) {
        ...
        switch (chunk.tupltype) {
            case 'blackandwhite' ://each pixel will only be 1 byte with value 0 or 1
                ...
                break;
            case 'grayscale' ://each pixel will only be 1 byte with value 0 to 255
                this._comparePixels = (oldPix, newPix) => {
                    let coords = [];
                    for (let i = 0, x = 0, y = 0, p = 0; i < length; i++, x++, p+=4) {
                        if (x === width) {
                ...
                break;
            case 'rgb' ://each pixel will be 3 bytes with value 0 to 255
            case 'rgb_alpha' ://each pixel will be 4 bytes with value 0 to 255
