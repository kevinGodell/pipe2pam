...
Pipe2Pam.prototype._findPam = function (chunk) {
  ...
  if (this._loi === this._chunksTotalLength) {
    ...
    const data = {
      pam: buffer,
      headers: buffer.slice(0, this._loh),
      pixels: buffer.slice(this._loh),
      width: this._headers.width,
      height: this._headers.height,
      depth: this._headers.depth,
      maxval: this._headers.maxval,
      tupltype: this._headers.tupltype,
      chunks: this._chunksLength,
    };
    ...
    for (let i = 0, soi = 0, sop = this._loh, eoi = this._loi; i < divided; i++, soi += this._loi, sop += this._loi, eoi += this._loi) {
      const data = {
        pam: buffer.slice(soi, eoi),
        headers: buffer.slice(soi, sop),
        pixels: buffer.slice(sop, eoi),
        width: this._headers.width,
        height: this._headers.height,
        depth: this._headers.depth,
        maxval: this._headers.maxval,
